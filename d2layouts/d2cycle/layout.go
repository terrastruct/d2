...
func moveObjectAndInternalEdges(obj *d2graph.Object, x, y float64) {
	...
	for _, edge := range obj.Graph.Edges {
		...
		case srcMoved && dstMoved:
			// The entire edge belongs to this child, so preserve its route shape
			// while translating it with the child.
			edge.Move(dx, dy)
		case srcMoved:
			// Cross-child descendant edges are laid out before the cycle nodes
			// move. Translate the endpoint that moved; the other endpoint will be
			// adjusted when its own cycle child is moved.
			edge.Route[0].X += dx
			edge.Route[0].Y += dy
		case dstMoved:
			i := len(edge.Route) - 1
