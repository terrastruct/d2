...
`), nil)
	...
	if x == nil || y == nil {
		t.Fatal("expected nested edge endpoints")
	}

	var edge *d2graph.Edge
	for _, candidate := range g.Edges {
		if candidate.Src == x && candidate.Dst == y {
			edge = candidate
			break
		}
	}
	...
	if len(edge.Route) != 2 {
		t.Fatalf("expected a two-point core route, got %d points", len(edge.Route))
	}
	if !samePoint(edge.Route[0], x.Center()) || !samePoint(edge.Route[1], y.Center()) {
		t.Fatalf("expected cross-child route to follow moved endpoints: route=%v src=%v dst=%v", edge.Route, x.Center(), y.Center())
	}
}
