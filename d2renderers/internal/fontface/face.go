...
func parseFaceCollectionWithLimitUsingParser(data []byte, maxFaces int, parseShaping shapingCollectionParser) (*Collection, error) {
	...
	return &Collection{outline: outline, shaping: shaping, digest: sha256.Sum256(data)}, nil
}
...
func (f *ParsedFace) Clone() (*ParsedFace, error) {
	...
	clone := &ParsedFace{Outline: f.Outline, Shaping: gotextfont.NewFace(f.Shaping.Font)}
	if f.hasParsedSourceDigest() {
		clone.source = &parsedFaceSource{
			digest:      f.source.digest,
			outline:     clone.Outline,
			shaping:     clone.Shaping,
			shapingFont: clone.Shaping.Font,
		}
	}
	return clone, nil
}

func (f *ParsedFace) hasParsedSourceDigest() bool {
	return f != nil && f.source != nil && f.Outline == f.source.outline && f.Shaping == f.source.shaping && f.Shaping != nil && f.Shaping.Font == f.source.shapingFont
}

func (f *ParsedFace) hasParsedSource(digest [sha256.Size]byte) bool {
	return f.hasParsedSourceDigest() && f.source.digest == digest
}
