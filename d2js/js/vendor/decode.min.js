export let BrotliDecode = (() => {
  const f = Int32Array.from([
    ...
    b = Int32Array.from([0, 3, 2, 1, 0, 0, 0, 0, 0, 0, 3, 3, 3, 3, 3, 3]),
    j = Int32Array.from([0, 0, 0, 0, -1, 1, -2, 2, -3, 3, -1, 1, -2, 2, -3, 3]),
    l = Int32Array.from([
  ...
  function s(f, w, b, j) {
    if (w < j + (2 << b)) return lf(f, -23);
    const l = 4 + ((w - j) >> b),
      m = g(l) - 1;
    return (((((m - 1) << 1) | ((l >> m) & 1)) - 1) << b) + (1 << b) + j + 16;
  }
  function t(f) {
    if (0 !== f.j) return lf(f, -26);
    (f.l = new Int32Array(3091)), (f.l[0] = 7), (f.m = 3);
    let w = s(f, 2147483644, 3, 120);
    if (w < 0) return w;
    const b = w;
    return (
      (f.p = new Int8Array(b)),
      (f.q = new Int32Array(b)),
      (w = (function (f) {
        return (
          (f.o = new Int8Array(4160)),
          (f.k = 0),
          (f.t = new Int16Array(2080)),
          (f.v = 32),
          (f.H = 2048),
          (f.u = 0),
          E(f)
  ...
  function v(f) {
    if (
      (f.v >= 16 && ((f.k = (f.t[f.H++] << 16) | (f.k >>> 16)), (f.v -= 16)),
      0 !== X(f, 1))
    ) {
      const w = X(f, 3);
      return 0 === w ? 1 : X(f, w) + (1 << w);
    }
    return 0;
  }
  function e(f, w, b) {
    ...
    const m = f[j] >> 16,
      p = 65535 & f[j];
    if (m <= 8) return (b.v += m), p;
    j += p;
    return (j += (l & ((1 << m) - 1)) >>> 8), (b.v += 8 + (f[j] >> 16)), 65535 & f[j];
  }
  function d(f, w, b) {
    b.v >= 16 && ((b.k = (b.t[b.H++] << 16) | (b.k >>> 16)), (b.v -= 16));
    const j = e(f, w, b),
      l = p[j];
    return (
      b.v >= 16 && ((b.k = (b.t[b.H++] << 16) | (b.k >>> 16)), (b.v -= 16)),
      m[j] + (l <= 16 ? X(b, l) : M(b, l))
    );
  }
  function H(f, w) {
    let b = w;
  ...
  function a(f, w, b, j, l) {
    const m = new Int32Array(w),
      p = new Int32Array(4),
      q = 1 + g(f - 1),
      o = X(l, 2) + 1;
    for (let f = 0; f < o; ++f) {
      l.v >= 16 && ((l.k = (l.t[l.H++] << 16) | (l.k >>> 16)), (l.v -= 16));
      const b = X(l, q);
      if (b >= w) return lf(l, -15);
      p[f] = b;
    }
    const n = (function (f, w, b) {
      for (let j = 0; j < b - 1; ++j)
        for (let l = j + 1; l < b; ++l) if (w[j] === w[l]) return lf(f, -7);
      return 0;
    })(l, p, o);
    if (n < 0) return n;
    let k = o;
    switch ((4 === o && (k += X(l, 1)), k)) {
      case 1:
        m[p[0]] = 1;
        break;
      case 2:
        ...
        break;
      case 3:
        (m[p[0]] = 1), (m[p[1]] = 2), (m[p[2]] = 2);
        break;
      case 4:
        (m[p[0]] = 2), (m[p[1]] = 2), (m[p[2]] = 2), (m[p[3]] = 2);
        break;
      case 5:
        (m[p[0]] = 1), (m[p[1]] = 2), (m[p[2]] = 3), (m[p[3]] = 3);
    }
    return C(b, j, 8, m, w);
  }
  function u(f, b, j, m, p) {
    const q = new Int32Array(f),
      o = new Int32Array(18);
    let n = 32,
      g = 0;
    for (let f = b; f < 18; ++f) {
      const b = w[f];
      p.v >= 16 && ((p.k = (p.t[p.H++] << 16) | (p.k >>> 16)), (p.v -= 16));
      const j = (p.k >>> p.v) & 15;
      p.v += l[j] >> 16;
      const m = 65535 & l[j];
      if (((o[b] = m), 0 !== m && ((n -= 32 >> m), g++, n <= 0))) break;
    }
    if (0 !== n && 1 !== g) return lf(p, -4);
    const k = (function (f, w, b, j) {
      let l = 0,
        m = 8,
        p = 0,
        q = 0,
        o = 32768;
      const n = new Int32Array(33);
      for (C(n, n.length - 1, 5, f, 18); l < w && o > 0; ) {
        if (j.H > 2030) {
          const f = J(j);
          if (f < 0) return f;
        }
        j.v >= 16 && ((j.k = (j.t[j.H++] << 16) | (j.k >>> 16)), (j.v -= 16));
        const f = (j.k >>> j.v) & 31;
        j.v += n[f] >> 16;
        const g = 65535 & n[f];
        if (g < 16) (p = 0), (b[l++] = g), 0 !== g && ((m = g), (o -= 32768 >> g));
        else {
          const f = g - 14;
          let n = 0;
          16 === g && (n = m), q !== n && ((p = 0), (q = n));
          const k = p;
          p > 0 && ((p -= 2), (p <<= f)),
            j.v >= 16 && ((j.k = (j.t[j.H++] << 16) | (j.k >>> 16)), (j.v -= 16)),
            (p += X(j, f) + 3);
          const s = p - k;
          if (l + s > w) return lf(j, -2);
          for (let f = 0; f < s; ++f) b[l++] = q;
          0 !== q && (o -= s << (15 - q));
        }
      }
      return 0 !== o ? lf(j, -18) : (b.fill(0, l, w), 0);
    })(o, f, q, p);
    return k < 0 ? k : C(j, m, 8, q, f);
  }
  function z(f, w, b, j, l) {
    ...
    l.v >= 16 && ((l.k = (l.t[l.H++] << 16) | (l.k >>> 16)), (l.v -= 16));
    const m = X(l, 2);
    return 1 === m ? a(f, w, b, j, l) : u(w, m, b, j, l);
  }
  function h(w, b, j) {
    let l;
    if (j.H > 2030 && ((l = J(j)), l < 0)) return l;
    const m = v(j) + 1;
    if (1 === m) return b.fill(0, 0, w), m;
    j.v >= 16 && ((j.k = (j.t[j.H++] << 16) | (j.k >>> 16)), (j.v -= 16));
    let p = 0;
    0 !== X(j, 1) && (p = X(j, 4) + 1);
    const q = m + p,
      o = f[(q + 31) >> 5],
      n = new Int32Array(o + 1),
      g = n.length - 1;
    if (((l = z(q, q, n, g, j)), l < 0)) return l;
    let k = 0;
    for (; k < w; ) {
      if (j.H > 2030 && ((l = J(j)), l < 0)) return l;
      j.v >= 16 && ((j.k = (j.t[j.H++] << 16) | (j.k >>> 16)), (j.v -= 16));
  ...
  function i(f, w, b) {
    ...
    f.v >= 16 && ((f.k = (f.t[f.H++] << 16) | (f.k >>> 16)), (f.v -= 16));
    let m = e(f.l, 2 * w, f);
    const p = d(f.l, 2 * w + 1, f);
    return (
      1 === m ? (m = j[l + 1] + 1) : 0 === m ? (m = j[l]) : (m -= 2),
      m >= b && (m -= b),
      (j[l] = j[l + 1]),
      (j[l + 1] = m),
      p
  ...
  function r(f) {
    f.i = i(f, 0, f.I);
    const w = f.h[5];
    (f.R = w << 6), (f.K = 255 & f.T[f.R]);
    const b = f.W[w];
    (f.B = b << 9), (f.Y = f.B + 256);
  }
  function I(f) {
    (f.A = i(f, 1, f.V)), (f.D = f.h[7]);
  }
  function R(f) {
    (f.F = i(f, 2, f.S)), (f.P = f.h[9] << 2);
  }
  function K(f) {
    if (0 !== f.U) return (f.C = 10), (f.j = 12), 0;
    let w;
    if (
      ((f.J = new Int32Array(0)),
      (f._ = new Int32Array(0)),
      (f.X = new Int32Array(0)),
      f.H > 2030 && ((w = J(f)), w < 0))
    )
      return w;
    if (
      ((w = (function (f) {
        ...
        const w = X(f, 2) + 4;
        if (7 === w) {
          if (((f.O = 1), 0 !== X(f, 1))) return lf(f, -6);
          const w = X(f, 2);
          if (0 === w) return 0;
          for (let b = 0; b < w; ++b) {
            f.v >= 16 && ((f.k = (f.t[f.H++] << 16) | (f.k >>> 16)), (f.v -= 16));
            const j = X(f, 8);
            if (0 === j && b + 1 === w && w > 1) return lf(f, -8);
            f.M += j << (8 * b);
          }
        } else
          for (let b = 0; b < w; ++b) {
            f.v >= 16 && ((f.k = (f.t[f.H++] << 16) | (f.k >>> 16)), (f.v -= 16));
            const j = X(f, 4);
            if (0 === j && b + 1 === w && w > 4) return lf(f, -8);
            f.M += j << (4 * b);
          }
        return f.M++, 0 === f.U && (f.G = X(f, 1)), 0;
      })(f)),
      w < 0)
    )
      return w;
    if (0 === f.M && 0 === f.O) return 0;
    if (0 !== f.G || 0 !== f.O) {
      if (((w = O(f)), w < 0)) return w;
      0 === f.O ? (f.j = 6) : (f.j = 5);
    } else f.j = 3;
    return (
      0 !== f.O ||
        ...
        f.L < f.N &&
          (function (f) {
            ...
            if (w > f.Z) {
              const b = f.Z;
              for (; w >> 1 > b; ) w >>= 1;
              0 === f.U && w < 16384 && f.N >= 16384 && (w = 16384);
            }
            if (w <= f.L) return;
            const b = new Int8Array(w + 37),
              j = f.$;
            0 !== j.length && b.set(j.subarray(0, f.L), 0), (f.$ = b), (f.L = w);
          })(f)),
      0
    );
  }
  function T(f, w, b) {
    let j = f.l[2 * w];
    if (b <= 1) return (f.l[2 * w + 1] = j), (f.l[2 * w + 2] = j), 1 << 28;
    const l = b + 2;
    let m = z(l, l, f.l, 2 * w, f);
    if (m < 0) return m;
    (j += m), (f.l[2 * w + 1] = j);
    return (
      (m = z(26, 26, f.l, 2 * w + 1, f)),
      m < 0 ? m : ((j += m), (f.l[2 * w + 2] = j), d(f.l, 2 * w + 1, f))
    );
  }
  function c(f) {
    f.I = v(f) + 1;
    let w = T(f, 0, f.I);
    if (w < 0) return w;
    if (((f.i = w), (f.V = v(f) + 1), (w = T(f, 1, f.V)), w < 0)) return w;
    if (((f.A = w), (f.S = v(f) + 1), (w = T(f, 2, f.S)), w < 0)) return w;
    if (((f.F = w), f.H > 2030 && ((w = J(f)), w < 0))) return w;
    f.v >= 16 && ((f.k = (f.t[f.H++] << 16) | (f.k >>> 16)), (f.v -= 16)),
      (f.ff = X(f, 2)),
      (f.wf = X(f, 4) << f.ff),
      (f.W = new Int8Array(f.I));
    let b = 0;
    for (; b < f.I; ) {
      const j = Math.min(b + 96, f.I);
      for (; b < j; )
        f.v >= 16 && ((f.k = (f.t[f.H++] << 16) | (f.k >>> 16)), (f.v -= 16)),
          (f.W[b] = X(f, 2)),
          b++;
      if (f.H > 2030 && ((w = J(f)), w < 0)) return w;
    }
    const j = f.I << 6;
    if (((f.T = new Int8Array(j)), (w = h(j, f.T, f)), w < 0)) return w;
    const l = w;
    f.bf = 1;
    for (let w = 0; w < j; ++w)
      if (f.T[w] !== w >> 6) {
    ...
    if (((f.jf = new Int8Array(f.S << 2)), (w = h(f.S << 2, f.jf, f)), w < 0)) return w;
    const m = w;
    if (((f.J = new Int32Array(B(256, l))), (w = Y(256, 256, l, f, f.J)), w < 0))
      return w;
    if (((f._ = new Int32Array(B(704, f.V))), (w = Y(704, 704, f.V, f, f._)), w < 0))
      return w;
    let p = k(f.ff, f.wf, 24),
      q = p;
    if (1 === f.lf) {
      if (((p = k(f.ff, f.wf, 62)), (w = s(f, 2147483644, f.ff, f.wf)), w < 0)) return w;
      q = w;
    }
    return (
      (f.X = new Int32Array(B(q, m))),
      (w = Y(p, q, m, f, f.X)),
      w < 0
        ? w
        : ((function (f, w) {
            ...
            let q = 1,
              o = 0,
              n = 16;
            for (let f = 0; f < m; ++f) (b[n] = 0), (j[n] = f + 1), ++n;
            for (; n < w; ) {
              const f = m + ((((2 + o) << q) - 4) << l) + 1;
              for (let w = 0; w < p; ++w) (b[n] = q), (j[n] = f + w), ++n;
              (q += o), (o ^= 1);
            }
          })(f, q),
          (f.R = 0),
  ...
  function y(f) {
    const w = f.$;
    let b;
    if (f.M <= 0) return (b = G(f)), b < 0 ? b : ((f.j = 2), 0);
    const j = Math.min(f.L - f.mf, f.M);
    return (
      (b = (function (f, w, b, j) {
        let l = b,
          m = j;
        if (7 & f.v) return lf(f, -30);
        for (; 32 !== f.v && 0 !== m; ) (w[l++] = f.k >>> f.v), (f.v += 8), m--;
        if (0 === m) return 0;
        const p = Math.min(Z(f), m >> 1);
        if (p > 0) {
          const b = f.H << 1,
            j = p << 1;
          w.set(f.o.subarray(b, b + j), l), (l += j), (m -= j), (f.H += p);
        }
        if (0 === m) return 0;
        if (Z(f) > 0) {
          for (
  ...
  function W(f) {
    const w = Math.min(f.pf - f.qf, f.nf - f.gf);
    return (
      0 !== w &&
        (f.kf.set(f.$.subarray(f.gf, f.gf + w), f.sf + f.qf), (f.qf += w), (f.gf += w)),
      f.qf < f.pf ? 0 : 2
    );
  }
  function B(w, b) {
  ...
  function A(f) {
    let w = f.L;
    return 0 !== f.tf && (w = Math.min(w, f.gf + f.pf - f.qf)), w;
  }
  function V(f, w) {
    if (f.vf > 2147483644) return lf(f, -9);
    const b = f.vf - f.maxDistance - 1 - f.ef;
    if (b < 0) {
      const w = (function (f, w, b) {
        -1 === f.df &&
          (function (f) {
            f.Hf = new Int8Array(256);
            let w = 8;
            for (; (f.ef - 1) >> w; ) w++;
            (w -= 8), (f.df = w);
            let b = 0,
      ...
      const j = N,
        l = f.rf;
      if (l > 31) return lf(f, -9);
      const m = ff[l];
      if (0 === m) return lf(f, -9);
      let p = $[l];
      const q = b >> m;
      p += (b & ((1 << m) - 1)) * l;
      const o = Q;
      if (q >= o.If) return lf(f, -9);
      const n = (function (f, w, b, j, l, m, p) {
        let q = w;
        const o = m.Rf,
          n = m.Kf,
          g = m.Tf,
        ...
        if (10 === t || 11 === t) {
          ...
          for (10 === t && (h = 1); h > 0; ) {
            const b = 255 & f[w];
            b < 192
              ? (b >= 97 && b <= 122 && (f[w] = 32 ^ f[w]), (w += 1), (h -= 1))
              : b < 224
              ? ((f[w + 1] = 32 ^ f[w + 1]), (w += 2), (h -= 2))
              : ((f[w + 2] = 5 ^ f[w + 2]), (w += 3), (h -= 3));
          }
        } else if (21 === t || 22 === t) {
          let w = q - h;
          const b = m.cf[p];
          let j = 16777216 - (32768 & b) + (32767 & b);
          for (; h > 0; ) {
            let b = 1;
            const l = 255 & f[w];
            if (l < 128) (j += l), (f[w] = 127 & j);
            else if (l < 192);
            else if (l < 224)
              if (h >= 2) {
                const m = f[w + 1];
      ...
      if (((f.mf += n), (f.M -= n), f.mf >= w)) return (f.C = 4), (f.j = 12), 0;
      f.j = 4;
    }
    return 0;
  }
  function D(f, w) {
    ...
    for (; f.hf !== f.if; ) {
      const j = w - b,
        l = f.af[f.uf + 1] - f.af[f.uf] - f.zf;
      let m = f.hf - f.if;
      if (
        (m > l && (m = l),
        m > j && (m = j),
        f.$.set(f.yf[f.uf].subarray(f.zf, f.zf + m), b),
        (b += m),
        (f.zf += m),
        (f.if += m),
        m === l && (f.uf++, (f.zf = 0)),
  ...
  function F(f) {
    let w;
    if (0 === f.j) return lf(f, -25);
    if (f.j < 0) return lf(f, -28);
    if (11 === f.j) return lf(f, -22);
    if (1 === f.j) {
      const w = (function (f) {
        const w = f.lf;
        if (
            ...
            0 !== b
              ? 1 === b
                ? 0 === w
                  ? -1
                  : ((f.lf = 1),
                    1 === X(f, 1) ? -1 : ((b = X(f, 6)), b < 10 || b > 30 ? -1 : b))
                : 8 + b
              : 17);
      })(f);
      if (-1 === w) return lf(f, -11);
      (f.N = 1 << w), (f.Wf = f.N - 16), (f.j = 2);
    }
    let l = A(f),
      m = f.L - 1,
      p = f.$;
    for (; 10 !== f.j; )
      switch (f.j) {
        case 2:
          if (f.M < 0) return lf(f, -10);
          if (((w = K(f)), w < 0)) return w;
          (l = A(f)), (m = f.L - 1), (p = f.$);
          continue;
        case 3:
          if (((w = c(f)), w < 0)) return w;
          f.j = 4;
          continue;
        case 4:
          if (f.M <= 0) {
          ...
          (f.Bf = n[q + 3]),
            f.v >= 16 && ((f.k = (f.t[f.H++] << 16) | (f.k >>> 16)), (f.v -= 16));
          const s = 255 & o;
          (f.Yf = g + (s <= 16 ? X(f, s) : M(f, s))),
            f.v >= 16 && ((f.k = (f.t[f.H++] << 16) | (f.k >>> 16)), (f.v -= 16));
          const t = o >> 8;
          (f.rf = k + (t <= 16 ? X(f, t) : M(f, t))), (f.Af = 0), (f.j = 7);
          continue;
        case 7:
          if (0 !== f.bf)
            for (; f.Af < f.Yf; ) {
            ...
            let b = 255 & p[(f.mf - 1) & m],
              j = 255 & p[(f.mf - 2) & m];
            for (; f.Af < f.Yf; ) {
              if (f.H > 2030 && ((w = J(f)), w < 0)) return w;
              0 === f.i && r(f);
              const m = x[f.B + b] | x[f.Y + j],
                q = 255 & f.T[f.R + m];
              if (
                (f.i--,
                (j = b),
          ...
          if (v < 0) f.vf = f.h[f.m];
            ...
            0 === f.F && R(f),
              f.F--,
              f.v >= 16 && ((f.k = (f.t[f.H++] << 16) | (f.k >>> 16)), (f.v -= 16));
            const l = 255 & f.jf[f.P + v];
            if (((v = e(f.X, l, f)), v < 16)) {
              const w = (f.m + b[v]) & 3;
              if (((f.vf = f.h[w] + j[v]), f.vf < 0)) return lf(f, -12);
            } else {
              const w = f.p[v];
              let b;
              f.v + w <= 32
          ...
          if ((v > 0 && ((f.m = (f.m + 1) & 3), (f.h[f.m] = f.vf)), f.rf > f.M))
            return lf(f, -9);
          (f.Af = 0), (f.j = 8);
          continue;
        case 8:
            ...
            if (a < 12 || (u > H && z > d)) {
              ...
              for (let w = 0; w < f; ++w)
                (p[H++] = p[d++]),
                  ...
                  (p[H++] = p[d++]),
                  (p[H++] = p[d++]);
            } else p.copyWithin(H, d, u);
            (f.Af += a), (f.M -= a), (f.mf += a);
          } else
            for (; f.Af < f.rf; )
              if (((p[f.mf] = p[(f.mf - f.vf) & m]), f.M--, f.mf++, f.Af++, f.mf >= l)) {
                (f.C = 8), (f.j = 12);
                break;
              }
          8 === f.j && (f.j = 4);
          continue;
        case 9:
          if (((w = V(f, l)), w < 0)) return w;
          continue;
        case 14:
          if (((f.mf += D(f, l)), f.mf >= l)) return (f.C = 14), (f.j = 12), 2;
          f.j = 4;
          continue;
        case 5:
          for (; f.M > 0; ) {
          ...
          continue;
        case 6:
          if (((w = y(f)), w < 0)) return w;
          continue;
        case 12:
          (f.nf = Math.min(f.mf, f.L)), (f.j = 13);
          continue;
        case 13:
          if (((w = W(f)), 0 !== w)) return w;
          f.mf >= f.Wf && (f.maxDistance = f.Wf),
            f.mf >= f.L &&
              (f.mf > f.L && p.copyWithin(0, f.L, f.mf), (f.mf = f.mf & m), (f.gf = 0)),
            (f.j = f.C);
          continue;
        default:
          return lf(f, -28);
    ...
    return 10 !== f.j
      ...
      : f.M < 0
      ? lf(f, -10)
      : ((w = O(f)), 0 !== w ? w : ((w = _(f, 1)), 0 !== w ? w : 1));
  }
  !(function (f) {
    const w = new Int32Array(24),
      b = new Int32Array(24);
    b[0] = 2;
    for (let f = 0; f < 23; ++f)
      (w[f + 1] = w[f] + (1 << q[f])), (b[f + 1] = b[f] + (1 << o[f]));
    for (let j = 0; j < 704; ++j) {
      let l = j >> 6,
        m = -4;
      l >= 2 && ((l -= 2), (m = 0));
      const p = (((170064 >> (2 * l)) & 3) << 3) | ((j >> 3) & 7),
        n = (((156228 >> (2 * l)) & 3) << 3) | (7 & j),
        g = b[n],
        k = m + Math.min(g, 5) - 2,
        s = 4 * j;
      (f[s] = q[p] | (o[n] << 8)), (f[s + 1] = w[p]), (f[s + 2] = b[n]), (f[s + 3] = k);
    }
  })(n);
  const Q = new (function (f, w, b) {
    (this.If = 0),
      (this.Rf = new Int32Array(0)),
      (this.Kf = new Int8Array(0)),
      (this.Tf = new Int32Array(0)),
      (this.cf = new Int16Array(0)),
      (this.If = f),
      (this.Rf = new Int32Array(3 * f)),
      (this.cf = new Int16Array(f)),
      (this.Kf = new Int8Array(w)),
      (this.Tf = new Int32Array(b + 1));
  })(121, 167, 50);
  function S(f, w) {
    let b = 1 << (w - 1);
    for (; f & b; ) b >>= 1;
  ...
  function C(f, w, b, j, l) {
    const m = f[w],
      ...
      q = new Int32Array(16),
      o = new Int32Array(16);
    for (let f = 0; f < l; ++f) q[j[f]]++;
    o[1] = 0;
    for (let f = 1; f < 15; ++f) o[f + 1] = o[f] + q[f];
    for (let f = 0; f < l; ++f) 0 !== j[f] && (p[o[j[f]]++] = f);
    let n = b,
      g = 1 << n,
      k = g;
    if (1 === o[15]) {
      for (let w = 0; w < k; ++w) f[m + w] = p[0];
      return k;
    }
    let s = 0,
      t = 0,
    ...
    for (let w = 1; w <= b; ++w)
      for (v <<= 1; q[w] > 0; )
        P(f, m + s, v, g, (w << 16) | p[t++]), (s = S(s, w)), q[w]--;
    const e = k - 1;
    let d = -1,
      H = m;
    v = 1;
    for (let w = b + 1; w <= 15; ++w)
      for (v <<= 1; q[w] > 0; )
        (s & e) !== d &&
          ((H += g),
          (n = U(q, w, b)),
          ...
          (d = s & e),
          (f[m + d] = ((n + b) << 16) | (H - m - d))),
          P(f, H + (s >> b), v, g, ((w - b) << 16) | p[t++]),
          (s = S(s, w)),
          q[w]--;
    return k;
  }
  function J(f) {
    if (0 !== f.u) return Z(f) >= -2 ? 0 : lf(f, -16);
    const w = f.H << 1;
    let b = 4096 - w;
    for (f.o.copyWithin(0, w, 4096), f.H = 0; b < 4096; ) {
      const w = 4096 - b,
        j = bf(f, f.o, b, w);
      if (j < -1) return j;
      if (j <= 0) {
    ...
    return (
      (function (f, w) {
        const b = f.o,
          j = w >> 1,
          l = f.t;
        for (let f = 0; f < j; ++f) l[f] = (255 & b[2 * f]) | ((255 & b[2 * f + 1]) << 8);
      })(f, b),
      0
    );
  }
  function _(f, w) {
    if (0 === f.u) return 0;
    const b = (f.H << 1) + ((f.v + 7) >> 3) - 4;
    return b > f.Vf ? lf(f, -13) : 0 !== w && b !== f.Vf ? lf(f, -17) : 0;
  }
  function X(f, w) {
    const b = (f.k >>> f.v) & ((1 << w) - 1);
    return (f.v += w), b;
  }
  function M(f, w) {
    const b = X(f, 16);
    return (
      (f.k = (f.t[f.H++] << 16) | (f.k >>> 16)), (f.v -= 16), b | (X(f, w - 16) << 16)
    );
  }
  function E(f) {
    if (f.H > 2030) {
  ...
  function O(f) {
    const w = (32 - f.v) & 7;
    if (0 !== w) {
      if (0 !== X(f, w)) return lf(f, -5);
    }
    return 0;
  }
  function Z(f) {
    let w = 2048;
    return 0 !== f.u && (w = (f.Vf + 1) >> 1), w - f.H;
  }
  !(function (f, w, b, j, l) {
    const m = jf(j),
      p = m.length;
    ...
    for (let f = 0; f < 363; ++f) b[f] = l.charCodeAt(f) - 32;
  })(
    Q.Kf,
    Q.Tf,
    Q.Rf,
  ...
  function L() {
    (this.$ = new Int8Array(0)),
      (this.W = new Int8Array(0)),
      (this.T = new Int8Array(0)),
      (this.jf = new Int8Array(0)),
      (this.p = new Int8Array(0)),
      (this.kf = new Int8Array(0)),
      (this.o = new Int8Array(0)),
      (this.t = new Int16Array(0)),
      (this.Df = new Int32Array(0)),
      (this.h = new Int32Array(0)),
      (this.l = new Int32Array(0)),
      (this.J = new Int32Array(0)),
      (this._ = new Int32Array(0)),
      ...
      (this.if = 0),
      (this.yf = new Array(0)),
      (this.af = new Int32Array(0)),
      (this.df = 0),
      (this.Hf = new Int8Array(0)),
      (this.input = new wf(new Int8Array(0))),
      (this.$ = new Int8Array(0)),
      (this.h = new Int32Array(10)),
      (this.h[0] = 16),
      (this.h[1] = 15),
      (this.h[2] = 11),
      (this.h[3] = 4);
  ...
  !(function (f, w, b) {
    for (let w = 0; w < 256; ++w)
      (f[w] = 63 & w), (f[512 + w] = w >> 2), (f[1792 + w] = 2 + (w >> 6));
    for (let b = 0; b < 128; ++b) f[1024 + b] = 4 * (w.charCodeAt(b) - 32);
    for (let w = 0; w < 64; ++w) (f[1152 + w] = 1 & w), (f[1216 + w] = 2 + (1 & w));
    let j = 1280;
    for (let w = 0; w < 19; ++w) {
      const l = 3 & w,
        m = b.charCodeAt(w) - 32;
      for (let w = 0; w < m; ++w) f[j++] = l;
    }
    for (let w = 0; w < 16; ++w) (f[1792 + w] = 1), (f[2032 + w] = 6);
    (f[1792] = 0), (f[2047] = 7);
    for (let w = 0; w < 256; ++w) f[1536 + w] = f[1792 + w] << 3;
  })(
    x,
    "         !!  !                  \"#$##%#$&'##(#)#++++++++++((&*'##,---,---,-----,-----,-----&#'###.///.///./////./////./////&#'# ",
    "A/*  ':  & : $  \x81 @"
  );
  let N = new Int8Array(0);
  const $ = new Int32Array(32),
    ff = new Int32Array(32);
  {
    const f = new Int8Array(122784),
      w = new Int32Array(25);
    !(function (f, w, b, j, l, m) {
      const p = (function (f) {
          const w = f.length,
            b = new Int8Array(w);
          for (let j = 0; j < w; ++j) b[j] = f.charCodeAt(j);
          return b;
        })(w + b),
        q = jf(j);
      let o = 0;
      ...
      for (let f = 0; f < n; ++f) {
        const w = q[2 * f] - 36,
          b = q[2 * f + 1] - 36;
        for (let f = 0; f < w; ++f) (p[o] = 3 ^ p[o]), o++;
        for (let f = 0; f < b; ++f) (p[o] = 236 ^ p[o]), o++;
      }
      for (let f = 0; f < m.length; ++f) l[f] = m.charCodeAt(f) - 65;
      f.set(p);
    })(
      f,
      ...
      (function (f, w) {
        const b = $,
          j = ff;
        for (let f = 0; f < w.length; ++f) j[f] = w[f];
        let l = 0;
        ...
        for (let f = w.length; f < 32; ++f) b[f] = l;
        N = f;
      })(f, w);
  }
  function wf(f) {
    (this.data = new Int8Array(0)), (this.offset = 0), (this.data = f);
  }
  function bf(f, w, b, j) {
    if (null === f.input) return -1;
    const l = f.input,
      m = Math.min(l.offset + j, l.data.length),
      p = m - l.offset;
    return w.set(l.data.subarray(l.offset, m), b), (l.offset += p), p;
  }
  function jf(f) {
    const w = f.length,
      b = new Int32Array(w);
  ...
  function lf(f, w) {
    if (w >= 0) return w;
    throw (f.j >= 0 && (f.j = w), new Error("Brotli error code: " + w));
  }
  return function (f, w) {
    let b = new L();
    if (((b.input = new wf(f)), t(b), w)) {
      ...
      f &&
        (function (f, w) {
          1 !== f.j
            ? lf(f, -24)
            : (0 === f.Qf &&
                ((f.yf = new Array(16)), (f.af = new Int32Array(16)), (f.df = -1)),
              15 === f.Qf
                ? lf(f, -27)
                : ((f.yf[f.Qf] = w), f.Qf++, (f.ef += w.length), (f.af[f.Qf] = f.ef)));
        })(b, f);
    }
    let j = 0,
      l = [];
    ...
    !(function (f) {
      0 === f.j ? lf(f, -25) : f.j > 0 && (f.j = 11);
    })(b),
      (function (f) {
        f.input = new wf(new Int8Array(0));
      })(b);
    let m = new Int8Array(j),
      p = 0;
    for (let f = 0; f < l.length; ++f) {
      let w = l[f],
        b = Math.min(j, p + 16384) - p;
      b < 16384 ? m.set(w.subarray(0, b), p) : m.set(w, p), (p += b);
    }
    return m;
  };
})();
