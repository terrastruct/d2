...
type Field struct {
	...
	// IsQuoted is also true if the field contains an escape.
	// For example, \label would have IsQuoted true
	IsQuoted bool `json:"-"`
...
func (m *Map) ensureField(i int, kp *d2ast.KeyPath, refctx *RefContext, create bool, gctx *globContext, c *compiler, fa *[]*Field) error {
	...
	for _, f := range m.Fields {
		if !(strings.EqualFold(f.Name, headString) && f.IsQuoted == headQuoted) {
			continue
		}
